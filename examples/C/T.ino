...
void list_test(void * param){
    ...
    lv_obj_set_size(list, 200, 200);
        ...
        while(true){
            for(int i = 0; i < 10; i++)
            //if(xSemaphoreTake(xSemaphore, portMAX_DELAY) == pdTRUE){
                lv_list_add_btn(list, NULL, "test");
                if(xSemaphoreTake(xSemaphore, portMAX_DELAY) == pdTRUE){
                    lv_obj_clean(list);
                    xSemaphoreGive(xSemaphore);
                }
...
void setup(){
   ...
   // contacts_list.setCheckPeriod(1);
    ...
    setupLvgl();
    xTaskCreatePinnedToCore(refresh, "refresh", 10000, NULL, 1, NULL, 1);
    xTaskCreatePinnedToCore(list_test, "list_test", 10000, NULL, 1, NULL, 1);
    //ui();
